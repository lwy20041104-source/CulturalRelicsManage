  ...
  <el-card class="view-card">
    <template #header>
      <div class="toolbar">
        ...
        <el-select v-model="query.priority" :placeholder="$t('repair.priority')" clearable style="width: 120px">
    ...
    <el-dialog v-model="materialsDialogVisible" :title="$t('repair.materialsUsed')" width="700px">
      ...
      <div v-if="materialsList.length === 0 && !materialsLoading" style="text-align: center; padding: 20px; color: #909399;">
        {{ $t('repair.noMaterials') }}
      </div>
      <template #footer>
        <el-button @click="materialsDialogVisible = false">{{ $t('common.close') }}</el-button>
      </template>
    </el-dialog>
  </el-card>
</template>
